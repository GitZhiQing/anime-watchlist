...
│   │   ├── store.ts             # tauri-plugin-store 封装（凭据/令牌/偏好）
│   │   └── utils.ts             # cn() 工具函数
│   ├── hooks/
...
本地仅存储 OAuth 凭据、访问令牌与界面偏好，不缓存业务数据（条目信息始终从 Bangumi API 实时获取）。

使用 `tauri-plugin-store` 持久化到 `settings.json`（Rust 侧 `lib.rs` 注册插件，前端通过 `@tauri-apps/plugin-store` 的 `LazyStore` 读写）。详见 `src/lib/store.ts`。
...
| `src-tauri/Cargo.toml` | `package.version` | Rust crate 版本，脚本自动同步 |
| `src-tauri/tauri.conf.json` | `version` | 应用版本，写入安装包元数据 |
...
npm run tauri build  # 前端构建 + Rust release 编译 + 打包（NSIS + MSI）
```
...
| Node.js | `actions/setup-node@v4`，版本来自 `.nvmrc`（22） |
| Rust | `actions-rust-lang/setup-rust-toolchain@v1` |
| 安装依赖 | `npm ci` |
| Tauri 构建 | `npm run tauri build`（NSIS `.exe` + MSI `.msi`） |
| 发布 | `softprops/action-gh-release@v2`，自动生成 release notes，上传安装包 |
